...
public class Ui_API {
	...
	public void hybridTest() throws InterruptedException {
		...
		Browser browser = pw.chromium()
				.launch(new BrowserType.LaunchOptions().setChannel("chrome").setSlowMo(100).setHeadless(false));
		...
		request = pw.request().newContext(new APIRequest.NewContextOptions()
				.setBaseURL("https://playwright-testleaf.atlassian.net").setExtraHTTPHeaders(headers));
